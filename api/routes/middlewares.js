        ...
        jwt.verify(token, secretKey, async (err, authorizedData) => {
            if (err) {
                ...
                if (authorizedData.role?._id) {
                    let actions = await RoleActions.find({ role: authorizedData.role?._id }).populate('action');
    ...
    if (typeof bearerHeader !== 'undefined') {
        ...
        try {
            ...
            if(checkAuthentication){
                ...
                return res.status(401).json({message: 'Chưa được đăng nhập vui lòng đăng nhập lại'})
            }
        } catch (error) {
            console.log(error)
